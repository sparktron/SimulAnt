...
// Coverage is measured as a RATE (unique surface tiles entered per window), not
// cumulative — cumulative unique coverage saturates near 100% of the surface over
// a long run and can't discriminate exploration efficiency. Windowed coverage
// captures ongoing spread: a clumping strategy covers fewer distinct tiles/window.
const COVERAGE_WINDOW = 500;
...
function runOne(seed, overrides) {
  ...
  // Coverage RATE: unique SURFACE tiles entered by SEARCHING (non-carrying) ants
  // per COVERAGE_WINDOW ticks, averaged over windows. The direct test of the
  // exploration hypothesis — does a mechanism spread searchers onto more distinct
  // ground per unit time? Windowed (not cumulative) because cumulative saturates.
  ...
  for (let tick = 1; tick <= TICKS; tick += 1) {
    ...
    for (const ant of colony.ants) {
      ...
      // Mark coverage for searchers on the surface (exploration, not return/nest).
      if (!carrying && !world.isBelowSurface(ant.x, ant.y) && world.inBounds(ant.x, ant.y)) {
        visited[world.index(ant.x, ant.y)] = 1;
      }
      if (carrying) {
        carryTicks += 1;
    ...
    // Close a coverage window: tally unique tiles this window, then reset the mask.
    if (tick % COVERAGE_WINDOW === 0) {
      let u = 0;
      for (let i = 0; i < visited.length; i += 1) { u += visited[i]; visited[i] = 0; }
      covSum += u; covWindows += 1;
    }
  }
  const coverage = covWindows ? covSum / covWindows : 0;
  return { nutrition, pickups, circling: circlingTicks, carryTicks, pr: prSamples ? prSum / prSamples : 0, finalAnts: colony.ants.length, coverage };
}
...
const agg = CONFIGS.map(() => ({ nut: 0, pick: 0, circ: 0, carry: 0, pr: 0, ants: 0, cov: 0 }));
...
CONFIGS.forEach((cfg, c) => {
  ...
  console.log(`${padE(cfg.name, 20)} ${pad(nut.toFixed(0), 8)} ${pad(c === 0 ? '—' : pct(gain), 10)} `
    ...
    + `${pad(cov.toFixed(0), 7)} ${pad(c === 0 ? '—' : pct(covDelta), 7)} `
