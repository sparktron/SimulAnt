...
test('Surface minimum zoom fits the true surface band into the canvas', () => {
  // surfaceHeight = nestY + 1 = 129 so the entire surface is visible at minZoom.
  // minZoom = 642 / 129 ≈ 4.977
...
test('Dirt-carrying ant deposits at surface near entrance', () => {
  ...
  const cfg = {
    ...
    hazardDeathChance: 0, digHomeBoost: 0.9, queenEggTicks: 20, queenEggFoodCost: 0.25,
    ...
    followAlpha: 1.5, followBeta: 5, wanderNoise: 0.06, momentumBias: 0.3, reversePenalty: 0.9, homeDepositIntervalTicks: 3,
...
test('Food-carrying ants must enter nest before depositing pellets', () => {
  ...
  const cfg = {
    tickSeconds: 1 / 30, maxFoodTrailScale: 1.2,
