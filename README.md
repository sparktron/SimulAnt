...
Current landed version: **v0.58.1**. The simulation includes deterministic
...
ends instead of repeatedly selecting an obstacle.
Queen-courier assignments now preserve food or dirt already in transit before
the worker collects a ration for the queen. Queen-food reservations also count
in-flight nurse deliveries and cannot exceed the queen's remaining deficit.
Ant decisions now distinguish completed local actions from failed movement, so
waiting, feeding, pickup, and deposit actions remain stationary; hazard checks
run after the final movement step.
Food respawning now monitors both colonies and centers relief near the colony
...
Emergency nest meals now preserve the ant's pre-meal starvation state, ensuring
the configured starvation health bonus is applied when the meal rescues it.
Critical queen-health requests assign two distinct nearest workers, allowing the
secondary courier slot to reinforce the primary delivery.
...
Mature brood now waits without consuming food when the ant cap is full, and
queenless save/load cycles preserve the elapsed worker-succession delay.
...
meters and populations update live.
...
- Two competing, queen-led colonies with independent populations, nests, food
  stores, and worker/soldier caste dynamics.
- Same-tile rival collisions have a 25% engagement chance; combat continues
  until one ant dies, and soldiers deal more damage than workers.
...
- **Caste allocation** (workers / soldiers; breeder lifecycle is reserved for a
  future feature)
...
Core controls and parameter labels use a larger default type scale, with larger
interactive targets for both desktop and coarse-pointer devices.
