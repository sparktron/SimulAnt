...
| **Two-pheromone recruitment** (separate short-lived recruit channel; `config.dualPheromone`, scaffold v0.49.0, A/B'd v0.49.1 via `bench/forage-sweep.mjs`) | net-NEGATIVE vs single mode at EVERY tuning. 12-seed: single +0.5%/pickups +9 vs best dual −3.8% to −6.1%/pickups −26 to −51. Stronger recruitment → monotonically worse (follow0.5 −37 → follow2 −142 pickups). Rich-source gating (`recruitRichOnly`) changed nothing (−85 vs −87 pickups un-gated) — falsified the "recruiting to depleted crumbs" hypothesis. | **The decisive lesson of this whole file.** This sim's bottleneck is *discovering relocating food*, i.e. EXPLORATION. Recruitment — to any source, fresh/rich or not — concentrates searchers on KNOWN clusters, trading exploration for exploitation; food depletes & respawns elsewhere, so that trade is always a loss. The "least-bad" dual config was the one that recruited LEAST (wide weak diff0.15); as recruitment→0, dual→single. Every win in this file (depletion-reactive decay) works by making trails LESS committal; recruitment is the opposite. The toggle/scaffold is kept (default off). |
| **Exploration / dispersion field** (repulsive `explored` channel; `config.explorationField`, scaffold v0.49.2–.3, A/B'd v0.49.3 via `bench/forage-sweep.mjs`) | net-NEGATIVE vs single at every dose. 12-seed: single +0.5%/pickups +9 vs best exploration (avoid0.1) −4.7%/pickups −19; stronger repulsion worse (avoid0.05 −94) and spikes circling to ~9–14% (ants flee their own marks — the repulsion twin of the death spiral). Coverage barely moved. NOTE: the pickup-triggered "dead-source repulsion" (role C) almost never fires — 396/397 pickups happen at 10+ pellet density; ants pick in abundance, so depletion is not observable at a pickup. Role B (searcher coverage) was the firing mechanism. | **Confirms the single path is at the environment's DISCOVERY CEILING.** Both *directions* of searcher steering now fail: attract toward food (recruitment) AND repel from worked ground (dispersion) both lose to doing nothing. When opposite interventions on the same variable both lose, that variable — searcher steering — is not the binding constraint. Pickups sit at ~900–910 regardless (OFF 880–903, single 890–907). The ceiling is set by food respawn rate / `foodVisionRadius` / ant count — ENVIRONMENT params, not pheromone behavior. To raise discovery, change the environment, not the trails. |
